...
仍須先完成上面的 UI 選配依賴安裝。若 8501 已由本 repository 先前啟動的
Streamlit 占用，啟動器會辨識 Windows venv 的父子程序鏈後安全重啟；若是其他
應用程式占用則拒絕終止。啟動器不會執行爬取或自動安裝套件。
