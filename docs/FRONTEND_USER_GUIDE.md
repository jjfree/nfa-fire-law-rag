...
在檔案總管中雙擊 `scripts\start_streamlit.bat`。啟動器會檢查專案的 `.venv` 與 Streamlit，啟動後自動開啟 `http://127.0.0.1:8501`。若 8501 已由本專案先前啟動的 Streamlit 占用，啟動器會安全重啟該程序；其他應用程式占用時則不會任意終止。啟動器不會自動安裝套件，也不會爬取法規。
...
| 設定 | 作用 | 使用建議 |
|---|---|---|
| 顯示結果數 | 設定一般查詢顯示的本機 RAG 證據數，範圍 1 至 20；Web 補充不計入此數量。完整列舉問題可能依章節自動超過此數量 | 一般問題保留預設 8；複雜比較可適度增加 |
| 法規篩選 | 限定只在指定法規中檢索 | 已知法規名稱時使用；跨法規問題保留「全部法規」 |
| gemma4:e2b | 本機速度優先；預設 8,192/2,048 tokens（context/output） | 快速查找與較短回答 |
| gemma4:e4b | 本機品質優先；預設 16,384/4,096 tokens（context/output） | 較完整的整理 |
| gemma4:31b-cloud | 雲端品質優先；預設 262,144/8,192 tokens（context/output） | 需要 `OLLAMA_API_KEY` |
...
啟動頁面、重新整理、送出問題完成回答，或從左側切換既有對話時，畫面會停留在該對話最後一輪的「問題」起點，再往下閱讀回答；不會自動捲到對話最底部。尚未提問的空白對話則維持目前頁面位置。
...
本機證據卡預設嚴格依 `[1]` 至 `[N]` 的引用編號順序顯示，即使同一法規出現多筆，也不會集中卡片而造成跳號；「顯示結果數」計算條文筆數。若另有 Web 補充，會接續編號，例如本機有 8 筆時，Web 證據從 `[9]` 開始。介面會另外顯示本機與 Web 的編號範圍。回答只需引用實際支持主張的證據，不一定會引用畫面上的每一張卡片。

當問題明確要求單一法規的「所有／全部／完整／逐條」懲處或罰則條文時，系統會改用結構式章節檢索，不以一般結果數截斷。畫面會顯示「完整列舉模式」、法規／章節與覆蓋數，例如 `21／21`；證據卡顯示「結構式章節檢索 · 依條文順序」，不顯示虛構的 hybrid 分數。只有 `returned_matches` 等於 `total_matches` 時，系統才標示完整。

對複合包含問題，直接涵蓋明確對象的條文優先，相關母法其次。卡片保留原始 hybrid、vector 與 lexical 分數；分數代表檢索相關度，不代表法律位階。

回答下方的診斷列顯示模型、實際輸出 token 數、有效輸出上限、中文生成狀態及重試次數。正常結束會顯示「生成狀態：已完成」，不直接呈現容易誤解的技術終止碼 `stop`；有效上限會依剩餘 context 縮小。若回答因長度中斷，系統會精簡重試一次，仍未完成時顯示「達到輸出上限，回答未完整」並明確警告。
...
| 現象 | 處理方式 |
|---|---|
| 瀏覽器無法開啟頁面 | 確認 Streamlit 視窗仍在執行，再開啟 `http://127.0.0.1:8501`；若啟動器顯示 8501 被其他程序占用，保留 PID 與錯誤訊息供管理者辨識 |
| 顯示端口被佔用 | 新版啟動器可辨識本專案 Windows venv 的父子程序鏈並安全重啟；若仍拒絕啟動，表示有限父鏈內無法確認是本專案程序，請勿直接終止不相干程式 |
...
| 顯示回答未完整 | 查看診斷列的生成狀態、實際輸出／上限與重試次數；可縮小問題範圍或減少顯示結果數後重問 |
...
| 日期 | 版本 | 異動說明 |
|---|---|---|
| 2026-09-24 | 1.9 | 修正 Windows venv 監聽子程序被誤判為外部程序，導致重複雙擊無法開啟頁面 |
| 2026-09-24 | 1.8 | 完整列舉問題改用結構式章節檢索，顯示章節範圍與證據覆蓋數 |
| 2026-09-22 | 1.7 | 問答載入、送出回答及切換對話時，視窗定位到最新問題起點 |
| 2026-09-17 | 1.6 | 證據卡改為依引用編號順序顯示，避免依法規分組造成跳號 |
| 2026-09-16 | 1.5 | 診斷列加入實際輸出 token 數，並將技術終止碼改為易懂的中文生成狀態 |
| 2026-09-16 | 1.4 | 加入模型別 token 上限、截斷重試/警告、證據依法規分組及跨法規來源多樣化說明 |
| 2026-09-16 | 1.3 | 說明直接條文、母法的證據順序及原始分數意義 |
