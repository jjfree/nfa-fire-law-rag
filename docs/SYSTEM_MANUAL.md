...
本手冊負責說明系統如何使用；後續程式異動的文件責任、對齊矩陣與交付檢查表，統一依 repository 根目錄的 [`AGENTS.md`](../AGENTS.md) 第 17 節執行。採影響範圍驅動的方式維護文件：只有影響操作、資料契約、來源規則、執行方式、資安邊界或驗證方式的變更，才需要同步受影響文件。
