...
證據卡片依編號排列：先列本機證據 `[1]` 至 `[top_k]`，再接續 Web 證據。每張卡片明確顯示 `[N]`、法規名稱、條號、標題、版本與 hybrid/vector/lexical 分數，畫面另列本機與 Web 的編號範圍。點擊卡片可展開原文；點擊回答中的 `[N]` 引用會跳到並展開對應證據。**分數是排序訊號，不是法律效力。**
