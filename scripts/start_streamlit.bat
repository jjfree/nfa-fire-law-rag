...
"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -Command "$repo = [IO.Path]::GetFullPath($env:NFA_REPO_DIR); $blocked = $false; $listeners = @(Get-NetTCPConnection -LocalPort ([int]$env:NFA_PORT) -State Listen -ErrorAction SilentlyContinue); foreach ($listener in $listeners) { $process = Get-CimInstance Win32_Process -Filter ('ProcessId=' + $listener.OwningProcess) -ErrorAction SilentlyContinue; $commandLine = [string]$process.CommandLine; if ($commandLine -and ($commandLine -like ('*' + $repo + '*') -and $commandLine -like '*streamlit*')) { Stop-Process -Id $listener.OwningProcess -Force -ErrorAction SilentlyContinue; Write-Output ('Stopped old NFA Streamlit process PID ' + $listener.OwningProcess) } elseif ($listener.OwningProcess) { Write-Output ('Port is owned by an unrelated process PID ' + $listener.OwningProcess); $blocked = $true } }; if ($blocked) { exit 2 }"
if errorlevel 2 (
