...
rem A Windows venv may leave the listening child on the base Python executable, so
rem inspect a bounded parent chain and terminate the matched repository ancestor.
rem If another application owns the port, fail closed instead of terminating it.
"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -Command "$repo = [IO.Path]::GetFullPath($env:NFA_REPO_DIR); $blocked = $false; $owners = @(); foreach ($line in @(netstat -ano)) { if ([string]$line -match (':' + $env:NFA_PORT + '.*LISTENING\s+(\d+)')) { $owners += [int]$Matches[1] } }; foreach ($owner in $owners) { $current = Get-CimInstance Win32_Process -Filter ('ProcessId=' + $owner) -ErrorAction SilentlyContinue; $matchedPid = 0; for ($depth = 0; $depth -lt 4 -and $current; $depth++) { $commandLine = [string]$current.CommandLine; if ($commandLine -and $commandLine -like ('*' + $repo + '*') -and $commandLine -like '*streamlit*') { $matchedPid = [int]$current.ProcessId; break }; $parentId = [int]$current.ParentProcessId; if (-not $parentId) { break }; $current = Get-CimInstance Win32_Process -Filter ('ProcessId=' + $parentId) -ErrorAction SilentlyContinue }; if ($matchedPid) { Start-Process -FilePath 'taskkill.exe' -ArgumentList @('/PID', [string]$matchedPid, '/T', '/F') -Wait -NoNewWindow; Write-Output ('Stopped old NFA Streamlit process tree PID ' + $matchedPid) } elseif ($owner) { Write-Output ('Port is owned by an unrelated process PID ' + $owner); $blocked = $true } }; if ($blocked) { exit 2 }"
if errorlevel 2 (
